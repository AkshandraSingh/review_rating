...
const imageConfig = multer.diskStorage({
    destination: (req, res, callback) => {
        callback(null, path.join(__dirname, "..", "/uploads")); // ! It Give Directory For Where Image is Store ( Company Profile Pic) .
    },
    filename: (req, file, callback) => {
        var ext = file.originalname.substring(file.originalname.indexOf("."));
        callback(null, `image_${Date.now()}.${file.originalname}`); // !  It Give Unique Name to Each Image ( Company Profile Pic) .
    }
})
